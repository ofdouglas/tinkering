...
class Regs:
    ...
    def dump_expected(self, path):
        lines = ["@00000000"] + [f"{self.x[i]:08x}" for i in range(32)]
...
if __name__ == "__main__":
    ...
    sim_csr_basic().dump_expected(root / "csr_basic.expected")
    sim_csr_immediate().dump_expected(root / "csr_immediate.expected")
    sim_csr_mixed().dump_expected(root / "csr_mixed.expected")
    print("wrote csr_*.expected")
